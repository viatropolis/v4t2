...
var ajaxChat = {
	...
	initialize: function() {	
		...
		//preload the console.log icon (it can't display if there's no connection unless it's cached!)
		this.setStatus('console.log');
	...
	setStatus: function(currentStatus) {
		...
		if (document.getElementById('statusIconContainer') != null ) {
			//currentStatus options are: Off for green, On for orange, and console.log for red.
	...
	loadFlashInterface: function() {
		if(this.dom['flashInterfaceContainer']) {
			this.updateDOM(
				...
				+'<param name="src" value="'+this.baseURL+this.dirs['flash']+'FABridge.swf"/>'
	...
	makeRequest: function(url, method, data) {
		...
		try {
			...
			if(data) {
				...
				ajaxChat.setStatus('console.log');
				this.updateChatlistView();
			}
	...
	handleXML: function(xmlDoc) {
		...
		this.handleOnlineUsers(userName);
		this.handleChatMessages(xmlDoc.getElementsByTagName('message'));
	...
	handleInfoMessage: function(infoType, infoData) {
		switch(infoType) {
			...
			case 'channelID':
				this.channelID = infoData;
				document.getElementById('booth').src = ajaxChatConfig.boothDir + this.channelID +'/index.html';
				if (this.channelID == 100)
					this.addChatBotMessageToChatList('[b]Welcome to Viatropolis. Please dont hesitate to ask a question.[/b]');
				if (this.channelID == 70)
					this.addChatBotMessageToChatList('[b]Welcome to Lounge. Have fun and enjoy your stay![/b]');
				if (this.channelID == 80)
					this.addChatBotMessageToChatList('[b]Welcome to Cafe. Sit back, have a bite to eat and enjoy your stay![/b]');
			...
			case 'userRole':
				...
				if(this.userRole == 2 || this.userRole == 3) {
					//document.getElementById('bbCodeContainer').style.width='400px';
					document.getElementById('settingsButton').style.visibility='visible';
					//document.getElementById('bbimg').style.display='block';
				}
				break;				
			case 'logout':
	...
	getUserNodeString: function(userID, userName, userRole) {
		if(this.userNodeString && userID == this.userID) {
			...
			var htmlName = this.formatText(userName);
			try{
				readyUsername=$(htmlName).text();
			} catch(e) {
				readyUsername=htmlName;
			}	
			if(htmlName.charAt(0)===".") readyUsername="."+readyUsername;
			if(readyUsername == "" || userName.charAt(0) == "." || readyUsername.substring(0,3) == "AFK") { 
				readyUsername = htmlName; 
				var anchor = "";
			} else {
				var href = $(htmlName).find("a").attr("href");
				var onclick = $(htmlName).find("a").attr("onclick");
				var color = $(htmlName).attr("style");			
				if(href!=readyUsername) {
					var anchor='<a href="'+href+'" onclick="'+onclick+'">[!]</a> ';
				}
			}
	...
	addMessageToChatList: function(dateObject, userID, userName, userRole, messageID, messageText, channelID, ip) {
		...
		var output = this.getChatListMessageString(
				dateObject, userID, userName, userRole, messageID, messageText, channelID, ip
		);
		this.DOMbuffer = this.DOMbuffer + output;
	...
	getChatListMessageString: function(dateObject, userID, userName, userRole, messageID, messageText, channelID, ip) {
		...
		var dateTime = this.settings['dateFormat'] ? '<span class="dateTime">'
			...
			if(htmlName.charAt(0)===".") readyUsername="."+readyUsername;
			if(readyUsername == "" || userName.charAt(0) == "." || userName.substring(0,4) == "AFK|") { 
				readyUsername = htmlName; 
				var anchor = "";
			} else {
				var href = $(htmlName).find("a").attr("href");
		...
		return	'<div id="'
				...
				+ ' dir="'
				+ this.baseDirection
				+ '" style="'+color+'" onclick="ajaxChat.insertText(\'/msg '+userName+' \');">'
				+ readyUsername
				+ '</span>'
				+ colon
	...
	replaceCommandLogin: function(textParts) {
		...
		return	'<span class="chatBotMessage">'
				+ this.lang['login'].replace(/%s/, this.formatText(textParts[1]))
				+ '</span>';		
	},
	...
	replaceCommandLogout: function(textParts) {
		...
		if(textParts.length == 3)
			type = textParts[2];
		return	'<span class="chatBotMessage">'
				+ this.lang['logout' + type].replace(/%s/, this.formatText(textParts[1]))
				+ '</span>';		
	},
	
	replaceCommandChannelEnter: function(textParts) {
		return	'<span class="chatBotMessage">'
				+ this.lang['channelEnter'].replace(/%s/, this.formatText(textParts[1]))
				+ '</span>';		
	},
	
	replaceCommandChannelLeave: function(textParts) {
		return	'<span class="chatBotMessage">'
				+ this.lang['channelLeave'].replace(/%s/, this.formatText(textParts[1]))
				+ '</span>';		
	},
	...
	replaceCommandPrivMsgTo: function(textParts) {
		...
		privMsgText = this.replaceEmoticons(privMsgText);
		return	'<span class="privmsg">'
				+ this.lang['privmsgto'].replace(/%s/, this.formatText(textParts[1]))
				+ '</span> '
				+ privMsgText;
	},
	...
	cChange:  function(type) {
			var clist = $("#channelSelection");
			var kids = clist.children();
			var first = 0;
			var last = kids.length-1;
			var verylast = kids.length;
			console.log("first: "+first+" last: "+last);
			var key=null;
			$.each(kids,function(i,o){
				$o = $(o);
				if(typeof $o.attr("selected") != 'undefined' && $o.attr("selected") == "selected") { key=i; }
			});
			if(key==null) {
				if(type=="up") key=last;
				if(type=="down") key=first;
			}
			var selected = $(kids[key]);
			var newselected;
			...
			if(type=="down") {
				if(key!=last)
					...
					newselected=$(kids[first]);
				newselected.attr("selected","selected");
			} else if(type=="up") {
				if(key!=first)
					newselected=selected.prev();
				else
					newselected=$(kids[last]);
				newselected.attr("selected","selected");
			}
			clist.val(newselected.val()); clist.change();
		},
