...
class AJAXChatTemplate extends Controller {
	...
	function replaceTemplateTags($tag, $tagContent) {
		switch($tag) {
			...
			case 'YII_BASE':
				return $this->ajaxChat->getConfig('baseUrlPath');
			case 'AURL':
				return $this->createAbsoluteUrl($tagContent);
	...
	function getStyleSheetLinkTags() {
		...
		foreach($this->ajaxChat->getConfig('styleAvailable') as $style) {
			...
			$styleSheets .= '<link rel="'.$alternate.'stylesheet" type="text/css" href="'.$this->ajaxChat->getConfig('baseUrlPath').'/css/'.$style.'.css'.'" title="'.$this->ajaxChat->htmlEncode($style).'"/>';
		}
		return $styleSheets;
	}
