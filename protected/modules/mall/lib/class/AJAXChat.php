...
class AJAXChat extends Controller {
	...
	function getDataBaseTable($table) {
		...
		return ($this->db->getName() 
			? $this->db->getName().'.'.$this->getConfig('dbTableNames',$table) 
			: $this->getConfig('dbTableNames',$table)
		);
	}
	...
	function initChatViewSession() {
		...
		if($this->getChannel() !== null) {
			...
			if(!$this->isUserOnline()) {
				$this->chatViewLogin(); #looks like error?
				return;
			}
	...
	function getTemplateFileName() {
		switch($this->getView()) {
			case 'chat':
				return AJAX_CHAT_PATH.'/views/loggedIn.php';
			case 'logs':
				return AJAX_CHAT_PATH.'/views/logs.php';
			default:
				return AJAX_CHAT_PATH.'/views/loggedOut.php';
		}
	}
...
#		$this->destroySession();
		...
		if(!$this->getConfig('allowNickChange') ||
			...
			if(!$newUserName) {
				...
				if($this->isUserNameInUse($newUserName)) {
					...
					$this->addInfoMessage($this->getUserName(), 'userName');
					$string = '/nick '.$oldUserName.' '.$newUserName;
					$this->insertChatBotMessage(
						$this->getChannel(),
						$string,
						null,
	...
	function startSession() {
		if(!session_id()) {
			...
			// Start the session:
			#session_start();
			// Session started by Yii.
			
			// We started a new session:...aka, YII did.
			$this->_sessionNew = true;
		}
	}
	
	function destroySession() {
		#if($this->_sessionNew) { <- always true.
	...
	function getSessionVar($key, $prefix=null) {
		...
		#if(isset(Yii::app()->session[$prefix.$key]))
		#	return Yii::app()->session[$prefix.$key];
		else
			return null;
	}
	...
	function setSessionVar($key, $value, $prefix=null) {
		...
		#Yii::app()->session[$prefix.$key]=$value;
	}
	...
	function getUserRole() {		
		...
		$user = $this->getValidLoginUserData();
